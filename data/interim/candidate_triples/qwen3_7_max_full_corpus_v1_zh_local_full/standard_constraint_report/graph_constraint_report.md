...
- 生成时间：`2026-08-09T16:21:07.872752+00:00`
...
|---|---:|---:|---:|---:|---:|
| KG_v1_raw | 8003 | 9631 | 7699 | 8003 | 8003 |
| KG_v1_validated | 1326 | 2022 | 1281 | 1326 | 1326 |
...
| `PKG002_UNIQUE_STABLE_IDENTIFIERS` 分层记录ID唯一且非空 | KG_v1_raw | pass | info | 33336 | 0 | 否 |
| `PKG003_LINK_TARGETS_AND_UNIQUENESS` Claim—Evidence链接稳定且目标存在 | KG_v1_raw | pass | info | 8003 | 0 | 否 |
| `PKG004_CLAIM_HAS_EVIDENCE` 每个Claim至少关联一条Evidence | KG_v1_raw | pass | info | 7699 | 0 | 否 |
...
| `PKG002_UNIQUE_STABLE_IDENTIFIERS` 分层记录ID唯一且非空 | KG_v1_validated | pass | info | 5955 | 0 | 否 |
| `PKG003_LINK_TARGETS_AND_UNIQUENESS` Claim—Evidence链接稳定且目标存在 | KG_v1_validated | pass | info | 1326 | 0 | 否 |
| `PKG004_CLAIM_HAS_EVIDENCE` 每个Claim至少关联一条Evidence | KG_v1_validated | pass | info | 1281 | 0 | 否 |
| `PKG005_EVIDENCE_HAS_CLAIM_LINK` 每条Evidence至少被一个Claim引用 | KG_v1_validated | pass | info | 1326 | 0 | 否 |
| `PKG006_SOURCE_RECORD_LINK_COVERAGE` 源记录与Claim—Evidence链接双向覆盖 | KG_v1_validated | pass | info | 2652 | 0 | 否 |
| `GOV001_ALLOWED_GOVERNANCE_STATUS` 治理状态属于冻结枚举 | KG_v1_validated | pass | info | 1326 | 0 | 否 |
| `SPLIT001_PRIMARY_GRAPH_BUILD_ONLY` 主图谱不含开发集或保留测试集 | KG_v1_validated | pass | info | 1326 | 0 | 否 |
| `GOV002_VALIDATED_ONLY_SILVER` 发布图只包含Silver记录 | KG_v1_validated | pass | info | 1326 | 0 | 否 |
| `PROV001_CORE_FIELDS_PRESENT` Silver/发布记录具有核心溯源字段 | KG_v1_validated | pass | info | 1326 | 0 | 否 |
| `PROV002_PAGE_URL_HASH_FORMAT` 物理页、URL和哈希格式有效 | KG_v1_validated | pass | info | 1326 | 0 | 否 |
| `PROV003_NONRELEASE_EVIDENCE_GAPS` 非发布记录的原文缺失审计 | KG_v1_validated | pass | info | 0 | 0 | 否 |
| `EVID001_RELEASE_E1_E2_NONINFERRED_ENTAILED` Silver/发布记录为有效E1/E2且非推断 | KG_v1_validated | pass | info | 1326 | 0 | 否 |
| `REL001_REGISTRY_DOMAIN_RANGE` Silver/发布关系符合注册表与Domain/Range | KG_v1_validated | pass | info | 1326 | 0 | 否 |
| `ID001_STABLE_SOURCE_IDENTIFIERS` Silver/发布源记录稳定ID可重算 | KG_v1_validated | pass | info | 1326 | 0 | 否 |
| `PKG007_LAYER_PROJECTION_CONSISTENCY` 源记录与实体/Claim/Evidence投影一致 | KG_v1_validated | pass | info | 1326 | 0 | 否 |
| `ZH001_RELEASE_ENDPOINT_GOVERNANCE` 发布端点满足中文术语门控 | KG_v1_validated | pass | info | 1326 | 0 | 否 |
| `ZH002_RELEASE_ENTITY_PROJECTION` 发布实体具有中文规范名和术语ID | KG_v1_validated | pass | info | 2022 | 0 | 否 |
| `RELEASE001_VALIDATED_SUBSET_WITH_IMMUTABLE_EVIDENCE` 发布记录可回溯到Raw且原文溯源未变 | KG_v1_raw -> KG_v1_validated | pass | info | 1326 | 0 | 否 |
...
- 示例：
  ...
  - `{"reason": "nonrelease_record_missing_evidence_text", "record": "MPT-2a279c111841c21b9d9e"}`
