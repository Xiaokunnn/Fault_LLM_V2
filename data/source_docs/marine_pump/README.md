...
- `heldout_archive_manifest_v1.json`：MP010—MP013保留测试PDF的文件级封存清单；这些文档已在上游方法冻结后进行隔离外部处理，但仍不进入主图或任何后续训练、调参和校准。
...
正式清单还必须保存稳定的 `source_family_id`、泵型、服务场景和适用范围。当前本地共有21份PDF、2096页。其中15份构建集文档共1934页；MP008为36页开发集；MP009—MP013共126页，为保留测试集。MP010—MP013已经在主图和研究点二协议冻结后完成一次隔离外部处理；不得因其已经解析而将其回流到主图、研究点三训练、课程设计、阈值选择或路由校准。MP017–MP021是在326页大候选池审计后，为五类明确缺口新增的官方厂商文档；MP022是在六类角色补缺达到7/10后，为干运转独立来源及剩余三类角色缺口新增的官方厂商文档。这些事后构建集补充必须披露，不能包装为盲测资料。文档身份、划分和纳入理由见：
