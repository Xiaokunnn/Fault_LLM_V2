...
2. Rebuild and replay the RP2 selector, verifier, and abstention policy on that exact graph before exporting teacher traces. The configured BGE index is currently missing locally, so freeze/replay must fail closed until rebuilt on a model-capable machine.
