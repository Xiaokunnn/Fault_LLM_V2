...
Invoke-Step "1/5 Build frozen held-out page plan" @("scripts/build_heldout_external_page_plan.py")
$Extraction = @(
...
if ($LocalOnly) {
    $Extraction += "--dry-run"
    Invoke-Step "2/5 Validate held-out inputs without API" $Extraction
...
try {
    ...
    Invoke-Step "2/5 Frozen extraction on MP010-MP013" $Extraction
    Invoke-Step "3/5 Frozen strict validation" @(
    ...
    Invoke-Step "4/5 Dual-pass external Silver adjudication" @(
    ...
    Invoke-Step "5/5 Prepare isolated outputs for both papers" @(
