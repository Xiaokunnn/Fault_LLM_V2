...
CONFIG="${RP2_CONFIG:-configs/rp2_graphrag_v4_faithfulness.json}"

echo "[RP2 v4 server] root=$ROOT"
echo "[RP2 v4 server] python=$($PYTHON --version 2>&1)"
echo "[RP2 v4 server] config=$CONFIG"
...
echo "========== 3/4 RP2 v4 retrieval, constrained Qwen generation and guard =========="
"$PYTHON" -u scripts/run_rp2_graphrag_v2.py \
  --config "$CONFIG" \
  --require-cuda \
  "$@"

echo "[RP2 v4 server] completed"
echo "[RP2 v4 server] results=results/experiments/research_point_2/graphrag_v4_faithfulness"
echo "Next: bash scripts/run_rp2_semantic_judge_v2_secure.sh"
