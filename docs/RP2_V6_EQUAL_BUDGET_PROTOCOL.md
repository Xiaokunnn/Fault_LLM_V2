...
所有基准相关性标签、引用质量和语义审计结论均为**自动基准标签或自动语义支持判断**。它们表示与冻结评价集及声明契约的一致性，不表示领域专家确认的事实正确性。历史目录名和冻结字段中的 `silver` 只作为复现标识保留。
...
5. 所有方法使用同一 Qwen2.5-7B 验证器、回答渲染器、输出契约和拒答规则。在线阶段不得读取 `relevant_evidence_ids` 或作为隐藏基准标签使用的 `fault_class_ids`。
...
每个原问题生成2个确定性改写，保持 \(f,r\) 和自动基准相关证据不变，只评价检索，不用改写数据调参：
...
该审计使用同一 `qwen3.7-max` 的两种独立提示词角色，只能称为**自动语义一致性审计**，不是“两个独立模型”或“专家审核”。历史脚本与物理字段继续保留 `silver` 名称以复现实验。API Key 通过隐藏输入临时注入，不得写入命令、配置或日志：
...
- 同预算 K3 方法在自动基准检索、引用和拒答指标上的差异；
- 新检索与本地7B生成逐行配对后的端到端时延；
- 角色、图传播以及完整系统模块的逐步增益；
- 开发集上的措辞稳健性和可复现的自动语义一致性审计。

不可报告：

- 把自动基准指标表述为Gold、专家真值或专家事实正确率；
