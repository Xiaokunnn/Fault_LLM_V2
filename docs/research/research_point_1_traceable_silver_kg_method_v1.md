...
| 图谱方法对照与消融实验 | 固定候选离线B0–Ours、关键消融和敏感性已完成 | 可写现有治理门控的结构效应；不能写成不同提示词的抽取精度对照 | 视论文主张决定是否补固定页面真实API对照 |
...
4. 已完成：在固定候选集合上完成 B0–Ours 累计治理对照、关键消融、文档/故障类聚类 bootstrap 和三组参数敏感性；若论文要主张提示词本身改善抽取，应另在固定分层页面子集上补真实 API 对照。
