...
The controlled benchmark contains 40 queries formed from ten pump fault classes and four diagnostic roles. The fixed evidence base can answer 34 queries; six intentionally test abstention. The online evaluation store contains 208 evidence assertions drawn from 14 source documents and grouped into ten source families. Relevance labels are hidden from all online methods. We also generated two deterministic paraphrases per query, yielding 80 untuned robustness queries. An isolated external set was processed only after protocol fixation; its seven answerable queries serve as a no-feedback check rather than a basis for statistical inference.

All primary methods use the same maximum budget $K=3$: \emph{Dense} uses only BGE-M3 similarity; \emph{Role} adds diagnostic-role gating; \emph{Role+Graph} adds one-hop propagation; and \emph{Full} additionally applies fault affinity, source novelty, active underfilling, and two-stage verification. A \emph{Full-NoGraph} control disables only graph propagation.

\subsection{Metrics and Runtime Protocol}

Retrieval is measured by Recall and NDCG. Response evaluation includes citation precision, recall, and F1; answerable-query recall; unanswerable-query abstention recall; and strict evidence support for atomic statements. Citation precision, recall, and F1 are macro-averaged independently over answerable queries; the reported F1 is not recomputed from the aggregate precision and recall. Efficiency measures include prompt tokens, model calls, stage latency, and end-to-end latency. We use 5,000 paired bootstrap samples clustered by fault class and report 95 percent confidence intervals (CIs).

The method targets resource-constrained deployment, but controlled experiments ran on one NVIDIA RTX 5880 Ada GPU with 48 GB memory. Qwen2.5-7B was loaded in BF16 entirely on the GPU, without CPU or disk offload. Each query was measured in three interleaved method orders with CUDA synchronization; quality uses one prespecified run and timing uses the per-query median. Latency covers retrieval, prompt construction, verification, and rendering, but excludes model loading, index construction, and warm-up. The claims therefore concern relative inference work and latency on identical hardware, not absolute embedded-device performance.
